...
export default class BackgroundColorEditing extends Plugin {
   ...
   constructor(editor) {
      ...
      editor.config.define('backgroundcolor', {
         options: [
   ...
   init() {
      ...
      editor.model.schema.extend('$text', {allowAttributes: 'backgroundcolor'});

      const options = editor.config.get('backgroundcolor.options');

      // Set-up the two-way conversion.
      editor.conversion.attributeToElement(_buildDefinition(options));

      editor.commands.add('backgroundcolor', new BackgroundColorCommand(editor));
   }
}
